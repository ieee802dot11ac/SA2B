...
BOOL OSLink(OSModuleInfo* newModule, void* bss) {
  ...
  if (newModule->version > 2 || (newModule->version >= 2 &&
       ((moduleHeader->align && (uintptr_t)newModule % moduleHeader->align != 0) ||
        (moduleHeader->bssAlign && (uintptr_t)bss % moduleHeader->bssAlign != 0)))) {
    return FALSE;
  }
  ...
  newModule->sectionInfoOffset += (uintptr_t)moduleHeader;
  moduleHeader->relOffset += (uintptr_t)moduleHeader;
  moduleHeader->impOffset += (uintptr_t)moduleHeader;

  for (i = 0; i < newModule->numSections; i++) {
    si = &OSGetSectionInfo(newModule)[i];
    if (si->offset != 0) {
      si->offset += (uintptr_t)moduleHeader;
    } else if (si->size != 0) {
      si->offset = (uintptr_t)bss;
      bss = (void*)((uintptr_t)bss + si->size);
    }
  }
  for (imp = (OSImportInfo*)moduleHeader->impOffset;
       imp < (OSImportInfo*)(moduleHeader->impOffset + moduleHeader->impSize); imp++) {
    imp->offset += (uintptr_t)moduleHeader;
  }
  ...
  if (__OSStringTable) {
    newModule->nameOffset += (uintptr_t)__OSStringTable;
  }
