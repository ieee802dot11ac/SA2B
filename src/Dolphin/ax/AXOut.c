...
inline void memcpy(void *dst, const void *src, int count) {
  while (count--) {
    *((char *)dst)++ = *((const char *)src)++;
  }
}

#define BZERO_4(buffer, size)    \
    {                                  \
        vu32* p = (u32*)buffer;        \
        int i;                         \
        for (i = 0; i < size; i++) {   \
...
void __AXOutInit(void) {
    ...
    __AXOutFrame = 0;
    BZERO_4(__AXOutBuffer, sizeof(__AXOutBuffer));
    DCFlushRange(__AXOutBuffer, sizeof(__AXOutBuffer));
    BZERO_4(__AXOutSBuffer, sizeof(__AXOutSBuffer));
